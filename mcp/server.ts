...
const server = new Server(
  { name: 'dumpster-directory-cms', version: '1.0.0' },
  { capabilities: { tools: {} } }
)
  ...
  try {
    switch (name) {
      ...
      case 'get_all_cities': {
        ...
        if (args.state_slug) query = query.eq('state_slug', args.state_slug as string)
      ...
      case 'get_cities_without_businesses': {
        ...
        const idsWithBusinesses = [...new Set((activeCityRows || []).map((r: { city_id: number }) => r.city_id))]
      ...
      case 'get_cities_with_low_business_count': {
        const maxCount = (args.max_count as number) ?? 3
        ...
        const result = (data || []).map((city: { id: number; [key: string]: unknown }) => ({
          ...city,
      ...
      case 'get_city': {
        const { data, error } = await db
          .from('cities')
          .select('*')
          .eq('state_slug', args.state_slug as string)
          .eq('city_slug', args.city_slug as string)
          .single()
        if (error) throw error
        return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] }
      }
      ...
      case 'search_cities': {
        const { data, error } = await db
          ...
          .ilike('city_name', `${args.query as string}%`)
          .order('population', { ascending: false })
      ...
      case 'get_businesses': {
        let query = db
          .from('businesses')
          .select('*')
          .eq('city_id', args.city_id as number)
      ...
      case 'create_business': {
        const { city_id, name, ...rest } = args as Record<string, unknown>
      ...
      case 'update_business': {
        ...
        if (fields.name) fields.slug = slugify(fields.name as string)
      ...
      case 'delete_business': {
        const { error } = await db.from('businesses').delete().eq('id', args.id as number)
        if (error) throw error
      ...
      case 'get_lead_cities_summary': {
        ...
        const summary: Record<string, { city: unknown; count: number }> = {}
        for (const lead of (data || [])) {
          const key = String(lead.city_id)
          if (!summary[key]) summary[key] = { city: lead.city, count: 0 }
          summary[key].count++
        }
